...
function openOrderModal(productId) {
  ...
  _set('modalProductName', p.name + ' — ' + p.price + ' so\'m/' + p.unit + ' (naqt)');
  _set('qtyUnit', p.unit);
...
function closeOrderModal() { _closeOverlay('orderModal'); }
...
function updateTotal() {
  ...
  if (raw && !String(_prod.price).includes('–') && !String(_prod.price).includes('-')) {
...
async function submitForm(e) {
  ...
  _hide('formSuccess'); _hide('formError');
  _setBtnState(btn, true, 'Yuborilmoqda...');
...
function prodCardHtml(p) {
  ...
  <div class="prod-card reveal" data-cat="${p.cat}">
    ...
    <div class="prod-body">
      ...
      ${p.price2 ? `<div style="font-size:11px;color:rgba(245,240,235,0.3);margin-top:2px;">
...
function shopCardHtml(p) {
  ...
  <div class="shop-card reveal" data-cat="${p.cat}">
    ...
    <div class="shop-body">
      ...
      ${p.price2 ? `<div style="font-size:11px;color:var(--sand);margin-bottom:10px;">
...
const _show = id => { const e = _el(id); if (e) e.style.display = 'block'; };
...
window.openOrderModal = function(id) {
  ...
  setTimeout(() => {
    document.querySelectorAll('#orderModal input[name="phone"]').forEach(setupPhone);
  }, 50);
};
...
function validatePhoneOnSubmit(form) {
  ...
  if (!validatePhone(phoneInp)) {
    ...
    if (!errMsg) {
      ...
      errMsg.style.cssText = 'font-size:11px;color:var(--rust);margin-top:4px;font-weight:600;';
      phoneInp.parentNode.appendChild(errMsg);
    }
    errMsg.textContent = '+998 dan keyin 9 ta raqam yozing';
    setTimeout(() => { if (errMsg) errMsg.textContent = ''; }, 3000);
    return false;
  }
...
const _phoneObserver = new MutationObserver(() => {
  document.querySelectorAll('input[name="phone"]:not([data-phone-init])').forEach(inp => {
